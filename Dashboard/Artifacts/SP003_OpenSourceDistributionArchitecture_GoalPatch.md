...
- Base Goal：`SP-003`，revision `2`（公开仓名称修订后的基础合同）；机器 patch：[SP003-GP-001.json](SP003_OpenSourceDistributionArchitecture_GoalPatch.json)。
- Base digest：由 [Goal Contract Base](SP003_OpenSourceDistributionArchitecture_GoalContract_Base.json) 的 `goal_patch.py validate-goal` 产生并绑定在机器 patch 中；名称修订后的当前摘要已重新计算。
