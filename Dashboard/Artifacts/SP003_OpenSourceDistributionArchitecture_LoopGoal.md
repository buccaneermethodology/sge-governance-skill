...
- 机器可读合同：[Goal Contract](SP003_OpenSourceDistributionArchitecture_GoalContract.json)；稳定策略：[Open-source Dual-repo Distribution KB](../../kb/docs/strategy/Open_Source_Dual_Repo_Distribution.md)；实现阶梯：[Goal Patch](SP003_OpenSourceDistributionArchitecture_GoalPatch.md)。
...
完整 ODA-MH-01..12 见 [Stage Plan](SP003_OpenSourceDistributionArchitecture_StagePlan.md) 与 [Goal Contract](SP003_OpenSourceDistributionArchitecture_GoalContract.json)。这些要求在设计时全部保留为 pending implementation，不得由单个 Session 或工具 PASS 关闭。
